...
1. 并行调度 **mc-compliance**（合规·硬门禁）+ **mc-grade**（质量·软评分），各输出结构化结论
   ...
   首轮为**全量评审**；第 2 轮起为**增量复核**（见步骤 e——防评审漂移：新鲜评审者
   逐轮开新口味意见会让 findings 永不收敛，只能靠 max 硬停）。
...
3. 否则（存在 `severity: block`，或任一 `verdict: revise`）：
   ...
   e. 重跑双审（**增量复核模式**：调度时携带上一轮两边 findings + 生产技能的修订对照。
      评审职责变为：① 逐条核验上轮 finding 是否已解决；② 对**改动段落**全量审查；
      ③ **未改动段落**仅 `severity: block` 级新发现才进 findings，新的 warn 级意见写入
      「备注（不触发回炉）」随结论带出，供暂停/放行时人工参考；④ 评分变动须可追溯到
      改动处或 finding 解决状态，不得因未改动内容的口味重估而波动），回到第 2 步。
